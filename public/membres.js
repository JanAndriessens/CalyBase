...
// Enhanced Excel file reader with HTML tag cleaning
async function readExcelFile(file) {
    return new Promise((resolve, reject) => {
        // Check if XLSX library is available
        if (typeof XLSX === 'undefined') {
...
// Continue Excel import with enhanced cleaning
window.continueExcelImport = async function() {
    try {
        const jsonData = window.tempExcelData;
        if (!jsonData) {
        ...
        const processedData = jsonData.slice(1).map((row, index) => {
            ...
            // Clean and extract data using column mapping
            const processedRow = {
                lifrasID: cleanExcelText(row[columnMapping.lifrasID]),
                nrFebras: cleanExcelText(row[columnMapping.nrFebras]),
                nom: cleanExcelText(row[columnMapping.nom]),
                prenom: cleanExcelText(row[columnMapping.prenom]),
                adresse: cleanExcelText(row[columnMapping.adresse]),
                codePostal: cleanExcelText(row[columnMapping.codePostal]),
                localite: cleanExcelText(row[columnMapping.localite]),
                email1: cleanExcelText(row[columnMapping.email1]),
                email2: cleanExcelText(row[columnMapping.email2]),
                email3: cleanExcelText(row[columnMapping.email3]),
                telephonePrive: cleanExcelText(row[columnMapping.telephonePrive]),
                telephoneBureau: cleanExcelText(row[columnMapping.telephoneBureau]),
                gsm1: cleanExcelText(row[columnMapping.gsm1]),
                gsm2: cleanExcelText(row[columnMapping.gsm2]),
                gsm3: cleanExcelText(row[columnMapping.gsm3]),
                dateCertificatMedical: cleanExcelText(row[columnMapping.dateCertificatMedical]),
                validiteCertificatMedical: cleanExcelText(row[columnMapping.validiteCertificatMedical]),
                dateECG: cleanExcelText(row[columnMapping.dateECG]),
                validiteECG: cleanExcelText(row[columnMapping.validiteECG]),
                ice: cleanExcelText(row[columnMapping.ice]),
                description: cleanExcelText(row[columnMapping.description]),
                pays: cleanExcelText(row[columnMapping.pays]),
                dateNaissance: cleanExcelText(row[columnMapping.dateNaissance]),
                lieuNaissance: cleanExcelText(row[columnMapping.lieuNaissance]),
                langue: cleanExcelText(row[columnMapping.langue]),
                nationalite: cleanExcelText(row[columnMapping.nationalite]),
                newsletter: cleanExcelText(row[columnMapping.newsletter]),
                typeCertif1: cleanExcelText(row[columnMapping.typeCertif1]),
                typeCertif2: cleanExcelText(row[columnMapping.typeCertif2]),
                plongeur: cleanExcelText(row[columnMapping.plongeur]),
                apneiste: cleanExcelText(row[columnMapping.apneiste]),
                gasBlender: cleanExcelText(row[columnMapping.gasBlender]),
                nitrox: cleanExcelText(row[columnMapping.nitrox]),
                plongeeSouterraine: cleanExcelText(row[columnMapping.plongeeSouterraine]),
                plongeurAdapte: cleanExcelText(row[columnMapping.plongeurAdapte]),
                qualificationPPA: cleanExcelText(row[columnMapping.qualificationPPA]),
                qualificationVE: cleanExcelText(row[columnMapping.qualificationVE]),
                techniqueSubaquatique: cleanExcelText(row[columnMapping.techniqueSubaquatique]),
                trimix: cleanExcelText(row[columnMapping.trimix]),
                dateDerniereInscription: cleanExcelText(row[columnMapping.dateDerniereInscription]),
                validiteCFPS: cleanExcelText(row[columnMapping.validiteCFPS]),
                adeps: cleanExcelText(row[columnMapping.adeps]),
                plongeeEnfantEncadrant: cleanExcelText(row[columnMapping.plongeeEnfantEncadrant]),
                photographe: cleanExcelText(row[columnMapping.photographe]),
                archeologue: cleanExcelText(row[columnMapping.archeologue]),
                medecin: cleanExcelText(row[columnMapping.medecin]),
                derniereModif: cleanExcelText(row[columnMapping.derniereModif]),
                licenceFD: cleanExcelText(row[columnMapping.licenceFD]),
                medical: ''
            };
...
async function importMembres(data) {
    try {
        ...
        for (const row of data) {
            ...
            // Check if avatar exists for this member
            if (row.lifrasID && existingAvatars.has(row.lifrasID)) {
                membreData.avatar = true;
                console.log('Found avatar for member:', row.lifrasID);
            } else {
                membreData.avatar = false;
            }
            ...
            if (batchCount >= BATCH_SIZE) {
                console.log(`Committing batch ${Math.ceil(count / BATCH_SIZE)} (${batchCount} documents)`);
                await newBatch.commit();
                batchCount = 0;
                // Create new batch for remaining documents
                const nextBatch = window.db.batch();
                Object.assign(newBatch, nextBatch);
            }
        }
        ...
        console.log(`Import complete! ${count} members imported.`);
